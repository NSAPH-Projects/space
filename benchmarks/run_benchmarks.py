...
def main(cfg: DictConfig) -> None:
    ...
    logfile = f"benchmarks/results/{cfg.algo.name}.jsonl"
    os.makedirs(os.path.dirname(logfile), exist_ok=True)
    LOGGER.info(f"Logging to {logfile}")

    # check if logfile exists, and if so, delete it
        ...
        for i, full_dataset in enumerate(env.make_all()):
            ...
            if len(param_space) > 0:
                ...
                objective = tune.with_resources(
                    objective, dict(cfg.algo.tune.resources)
                )
                ...
                LOGGER.info(
                    "...skipping hyperparameter tuning, no param space provided"
                )
                best_params = {}
